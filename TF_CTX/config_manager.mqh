...
STimeframeConfig CConfigManager::ParseTimeframeConfig(CJAVal *tf_config)
{
    ...
    if(ind_array != NULL)
    {
        for(int i=0;i<ind_array.Size();i++)
        {
            ...
            string col=ind["LevelsColorStr"].ToStr();
            if(StringLen(col)>0) icfg.levels_color=StringToColor(col);
            ...
            icfg.extensions_color = (color)ind["ExtensionsColor"].ToInt();
            col=ind["ExtensionsColorStr"].ToStr();
            if(StringLen(col)>0) icfg.extensions_color=StringToColor(col);
            icfg.extensions_style = StringToLineStyle(ind["ExtensionsStyle"].ToStr());
            icfg.extensions_width = (int)ind["ExtensionsWidth"].ToInt();
            icfg.parallel_color = (color)ind["ParallelColor"].ToInt();
            col=ind["ParallelColorStr"].ToStr();
            if(StringLen(col)>0) icfg.parallel_color=StringToColor(col);
            ...
            icfg.labels_color = (color)ind["LabelsColor"].ToInt();
            col=ind["LabelsColorStr"].ToStr();
            if(StringLen(col)>0) icfg.labels_color=StringToColor(col);
