...
public:
  ...
  double GetValue(int shift = 0); // returns support value
...
void CSupRes::UpdatePatternCounters(bool is_support, int index)
{
  ...
  {
    m_sup_touches++;
    if (IsBullPinBar(m_opens, m_closes, m_highs, m_lows, index))
      m_sup_pinbar++;
    if (IsBullEngulf(m_opens, m_closes, index))
      m_sup_engulf++;
    if (IsDoji(m_opens, m_closes, m_highs, m_lows, index))
      m_sup_doji++;
    if (IsBullMarubozu(m_opens, m_closes, m_highs, m_lows, index))
      m_sup_maru_bull++;
    if (IsBearMarubozu(m_opens, m_closes, m_highs, m_lows, index))
      m_sup_maru_bear++;
    if (IsInsideBar(m_highs, m_lows, index))
      m_sup_insidebar++;
    if (IsOutsideBar(m_highs, m_lows, index))
      m_sup_outsidebar++;
  }
  else
  {
    m_res_touches++;
    if (IsBearPinBar(m_opens, m_closes, m_highs, m_lows, index))
      m_res_pinbar++;
    if (IsBearEngulf(m_opens, m_closes, index))
      m_res_engulf++;
    if (IsDoji(m_opens, m_closes, m_highs, m_lows, index))
      m_res_doji++;
    if (IsBullMarubozu(m_opens, m_closes, m_highs, m_lows, index))
...
void CSupRes::CalculatePriceZones(const double &highs[], const double &lows[], int bars, double range)
{
  ...
  for (int i = 0; i < ArraySize(pivots); i++)
  {
    ...
    for (int j = 0; j < ArraySize(zones); j++)
    {
      ...
      {
        if (price < zones[j].lower)
          zones[j].lower = price;
        if (price > zones[j].upper)
          zones[j].upper = price;
        zones[j].touches++;
        added = true;
        break;
